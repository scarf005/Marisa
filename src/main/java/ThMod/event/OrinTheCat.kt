...
*/
        ...
        if (orin != null) {
            ThMod.logger.info("OrinTheCat : orin get : " + orin.name)
        } else {
            ThMod.logger.info("OrinTheCat : error : null orin")
        }
    }
    ...
    override fun onEnterRoom() {
        ThMod.logger.info("OrinTheCat : OnEnterRoom")
        ...
        when (screen) {
            CurScreen.INTRO -> when (buttonPressed) {
                0 -> if (satori) {
                    ...
                    ThMod.logger.info("OrinTheCat : INTRO : Skipping fight!")
                    screen = CurScreen.END
                    /*
              if (orin != null) {
              ...
              */roomEventText.updateBodyText(DESCRIPTIONS[4])
                    ...
                    ThMod.logger.info("OrinTheCat : INTRO : Fight!")
            ...
            CurScreen.PRE_COMBAT -> {
                ThMod.logger.info("OrinTheCat : PreCombat : Adding Reward")
                ...
                ThMod.logger.info("OrinTheCat : PreCombat : Entering combat")
                enterCombat()
                AbstractDungeon.lastCombatMetricKey = "Orin"
            }

            CurScreen.END -> {
                ThMod.logger.info("OrinTheCat : end : Opening Map")
                openMap()
            }
        }
    }

package marisa.relics.deprecated;
...
public class Cape extends CustomRelic {
  ...
  public void update() {
    ...
    if ((this.counter <= 0) || (this.usedUp)) {
      MarisaMod.logger.info("Cape_1 : update : returning for being used");
      return;
    }
    if (!this.isObtained) {
      MarisaMod.logger.info("Cape_1 : update : returning for not obtained");
      return;
    }
    if ((this.RclickStart) && (InputHelper.justReleasedClickRight)) {
      if (this.hb.hovered) {
        MarisaMod.logger.info("Cape_1 : update : hovered");
        this.Rclick = true;
      }
      this.RclickStart = false;
    }
    if ((this.isObtained) && (this.hb != null) && (this.hb.hovered)
      ...
      MarisaMod.logger.info("Cape_1 : update : right click detected");
      this.RclickStart = true;
    }
    if (this.Rclick) {
      MarisaMod.logger.info("Cape_1 : update : Calling OnRclick");
      this.Rclick = false;
      OnRclick();
    }
  }
